...
def divide_dataset(filenames):
    ...
    :param filenames: x1_name & y1: all data selected from original 2 dataset
                      x2, y2: x_generated & y_generated with label 3
    ...
    for i in range(0, y1.shape[0]):
        ...
        if y1[i][0] != 1:
            image = read_image(x1_names[i][0], is_covid=False)
        else:  # x2 only has generated data with label 3
            image = read_image(x1_names[i][0], is_covid=True)
        x = x + image
        y.append(y1[i][0])
...
if __name__ == '__main__':
    ...
    trainset, testset = divide_dataset(['./dataset/xray/initial/' + 'x_filenames.npy',
                                        './dataset/xray/initial/' + 'x_generated.npy',
                                        './dataset/xray/initial/' + 'y_original.npy',
                                        './dataset/xray/initial/' + 'y_generated.npy', ])

    np.save('./dataset/xray/x_train.npy', trainset[0])
    np.save('./dataset/xray/y_train_original.npy', trainset[1])
    np.save('./dataset/xray/x_test.npy', testset[0])
    np.save('./dataset/xray/y_test_original.npy', testset[1])
    trainset[1][trainset[1] == 3] = 1
    testset[1][testset[1] == 3] = 1
